---
title: "Land use alters ecosystem transfer of high value fatty acids by aquatic insects"
output:
  html_document:
    figure_caption: yes
    highlight: tango
...
Calculate amount of FA in sample
```{r, message=FALSE, warning=FALSE}
...
Add column dry_weight_mg, total_fa, weight_FA
```{r, message=FALSE, warning=FALSE}
...
order_emergence_hgam <- filter(order_emergence_hgam, 
                          family == "Chironomidae" | 
                          order %in% c("Ephemeroptera", "Plecoptera", "Trichoptera"))
```
...
## Fit HGAM for land use (forest, agriculture)
...
#  geom_point(data =bio_position_S_stat_sig, aes(x=day, y=fit, col = position), show.legend = FALSE) +
  ...
  labs(title = "Total fatty acids", 
       ...
       y=expression(Fatty~ acids ~"("~"µ"~g~ ~m^-2~d^-1~")")) +
...
## Fit HGAM for order in agriculture/forest
...
fa_position_order_S_stat$position <- gsub("vine","agriculture", fa_position_order_S_stat$position)
...
legend_data <- as.data.frame(cbind(x,y))
legend_data$position <- with(legend_data, ifelse(y == 0.2, "forest", "agriculture"))
...
legend_plot <- ggplot() +
  ...
  geom_line(data=legend_data, aes(x=x, y=y, col = position), show.legend = FALSE,
            size = 0.8) +
  scale_color_manual(values=c("deepskyblue", "chartreuse4")) +
  annotate(geom = "text", x = 182, y = 0.2, label = "forest", size = 6) +
  annotate(geom = "text", x = 192, y = 0.1, label = "agriculture", size = 6) +
  ylim(0, 0.3) +
  xlim(91, 256)
```
...
### Save plot
```{r, message=FALSE, warning=FALSE}
plot_fa <- ggarrange(fa_position_S_plot, 
                     fa_position_Ephemeroptera_S_plot,
...
#png("gam_plot_fa.png", width=10, height=15, units="in", res=300)
plot_fa
#dev.off()
```
...
## Export HGAM output 
```{r, message =FALSE, warning= FALSE}
#htmlreg(list(fa_position_S, fa_position_order_S), digits = 4, stars = #numeric(0), caption.above = TRUE, 
...
#htmlreg(list(fa_position_I, fa_position_order_I), digits = 4, stars = numeric(0), caption.above = TRUE, 
#        caption = "HGAM biomass", 
#        float.pos = "h!", label = "tab:3", 
